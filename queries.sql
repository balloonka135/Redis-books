...
/*
    9) for each user select books with same language
       and that aren't older then 20 years and the rating is > 4
*/
SELECT USERS.NAMEUSER, BOOKS.TITLE FROM USERS, BOOKS, RATINGS
WHERE USERS.NATIVELANGUAGE = BOOKS.LANGUAGE AND BOOKS.YEAROFPUBLICATION >= 1999
WHERE RATINGS.SK_IDBOOK = BOOKS.SK_IDBOOK AND USERS.SK_IDUSER = RATINGS.SK_IDUSER AND RATINGS.RATING > 4
